    ...
    var AnimationState = (function() {
        ...
        for (var i = 0, n = tracks.length; i < n; i++) {
          ...
          if (mix == 1) {
            for (var ii = 0; ii < timelineCount; ii++)
              timelines[ii].apply(
                skeleton, animationLast, animationTime,
                events, 1, core.MixPose.setup, core.MixDirection.in
              )
          } else {
            ...
            for (var ii = 0; ii < timelineCount; ii++) {
              ...
              if (timeline instanceof core.RotateTimeline) {
                this.applyRotateTimeline(
                  timeline, skeleton, animationTime, mix, pose,
                ...
                timeline.apply(
                  skeleton, animationLast, animationTime,
                  events, mix, pose, core.MixDirection.in
                )
              }
            }
          }
      ...
      AnimationState.prototype.applyMixingFrom = function(to, skeleton, currentPose) {
        ...
        for (var i = 0; i < timelineCount; i++) {
          ...
          if (timeline instanceof core.RotateTimeline)
            this.applyRotateTimeline(
              timeline, skeleton, animationTime, alpha, pose,
              timelinesRotation, i << 1, firstFrame
            )
          else {
            timeline.apply(
              skeleton, animationLast, animationTime,
              events, alpha, pose, core.MixDirection.out
            )
          }
        }
        if (to.mixDuration > 0)
      ...
      AnimationState.prototype.applyRotateTimeline = function(
        timeline, skeleton, time, alpha, pose, timelinesRotation, i, firstFrame
      ) {
        if (firstFrame)
          ...
          for (var slotName in map.slots) {
            ...
            for (var timelineName in slotMap) {
              ...
              if (timelineName == "attachment") {
                ...
                timelines.push(timeline)
                duration = Math.max(
                  duration, timeline.frames[(timeline.getFrameCount() - 1) * core.ColorTimeline.ENTRIES]
                )
                ...
                for (var i = 0; i < timelineMap.length; i++) {
                  ...
                  timeline.setFrame(
                    frameIndex, valueMap.time, light.r, light.g, light.b,
                ...
                timelines.push(timeline)
                duration = Math.max(
                  duration, timeline.frames[(timeline.getFrameCount() - 1) * core.TwoColorTimeline.ENTRIES]
                )
              } else
                throw new Error(
          ...
          for (var boneName in map.bones) {
            ...
            for (var timelineName in boneMap) {
              ...
              if (timelineName === "rotate") {
                ...
                timelines.push(timeline)
                duration = Math.max(
                  duration, timeline.frames[(timeline.getFrameCount() - 1) * core.RotateTimeline.ENTRIES]
                )
              } else if (
        ...
        if (map.ik) {
          for (var constraintName in map.ik) {
            ...
            for (var i = 0; i < constraintMap.length; i++) {
              ...
              timeline.setFrame(
                frameIndex, valueMap.time, this.getValue(valueMap, "mix", 1),
        ...
        if (map.transform) {
          for (var constraintName in map.transform) {
            ...
            for (var i = 0; i < constraintMap.length; i++) {
              var valueMap = constraintMap[i]
              timeline.setFrame(
                frameIndex, valueMap.time, this.getValue(valueMap, "rotateMix", 1),
            ...
            for (var timelineName in constraintMap) {
              ...
              if (timelineName === "position" || timelineName === "spacing") {
                ...
                duration = Math.max(
                  duration,
                  timeline.frames[
                    (timeline.getFrameCount() - 1) * core.PathConstraintPositionTimeline.ENTRIES
                  ]
                )
                ...
                duration = Math.max(
                  duration,
                  timeline.frames[
                    (timeline.getFrameCount() - 1) * core.PathConstraintMixTimeline.ENTRIES
                  ]
                )
              }
            }
            ...
            if (line.length == 0)
              ...
              textureLoader(line, function(texture) {
                ...
                if (!page.width || !page.height) {
                  ...
                  if (!page.width || !page.height) {
                    console.log(
                      "ERROR spine atlas page " + page.name +
                      ": meshes wont work if you dont specify size in atlas " +
                      "http://www.html5gamedevs.com/topic/18888-pixi-spines-and-meshes/?p=107121)"
                    )
                  }
                }
                iterateParser()
  ...
  function atlasParser() {
    return function(resource, next) {
      ...
      var adapter = metadata.images ? staticImageLoader(metadata.images) :
        metadata.image ? staticImageLoader({
          'default': metadata.image
        }) : metadata.imageLoader ?
          metadata.imageLoader(this, resource.name + '_atlas_page_', baseUrl, imageOptions) :
